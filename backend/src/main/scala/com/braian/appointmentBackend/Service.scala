...
object Service:
  def getAppointments(user: String) =
    ZIO.service[Service] >>= (_.getAppointments(user))

  def requestAppointment(user: String, desiredTime: Instant, doctorName: String) =
    ZIO.service[Service] >>= (_.requestAppointment(user, desiredTime, doctorName))

case class LiveService(m: Ref[Map[String, List[Long]]], appointments: Ref[Map[Long, Appointment]], random: Random.Service) extends Service:
